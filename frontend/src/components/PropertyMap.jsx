...
const MapUpdater = ({center, zoom, bounds}) => {
    ...
    useEffect(() => { // Forcer le rafraîchissement de la taille pour mobile
        map.invalidateSize();
...
const PropertyMap = ({properties, selectedRegion, userLocation}) => { // Centre de la Guinée (Conakry) par défaut
    ...
    useEffect(() => {
        fetch('/data/conakry_boundaries.json')
    ...
    if (selectedRegion && REGION_COORDINATES[selectedRegion.name]) {
        ...
        mapZoom = 13;
        // Good zoom level to see a neighborhood

        // Calculate bounds to include user and properties
        const locs = properties.filter(p => p.latitude && p.longitude).map(p => [p.latitude, p.longitude]);
        ...
        <div className="h-[350px] sm:h-[500px] rounded-2xl overflow-hidden border border-gray-100 shadow-inner">
            <MapContainer center={defaultCenter}
                ...
                <LayersControl position="topright">
                    <LayersControl.BaseLayer checked name="Plan Standard (OSM)">
                    ...
                    <LayersControl.BaseLayer name="Plan Clair (Lisible)">
                    ...
                    <LayersControl.Overlay checked name="Limites Administratives">
                        {
                        geoJsonData && (
                            <GeoJSON
                                data={geoJsonData}
                                ...
                                onEachFeature={(feature, layer) => {
                                    if (feature.properties && feature.properties.name) {
                                        layer.bindTooltip(feature.properties.name, {
                                            permanent: false,
                                            direction: "center",
                                            className: "bg-white px-2 py-1 border border-gray-200 rounded shadow-sm font-bold text-xs"
                                        });
                                    }
                                }}
                            />
                    ...
                    if (property.latitude && property.longitude) {
                        return (
                            <Marker key={
                                ...
                                icon={
                                    getMarkerIcon(property.property_type)
                            }>
                                <Popup maxWidth={280}
                                    autoPan={true}
                                    autoPanPadding={
