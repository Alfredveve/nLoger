...
describe('PropertyDetails Component', () => {
    beforeEach(() => {
        ...
        useAuthMock = {
            user: mockUser
        }; // Reset to logged in by default
    });

    it('renders loading state initially', () => { // Return a promise that never resolves immediately to test loading state
...
describe('PropertyDetails Integration', () => {
    it('displays property details after fetching', async () => {
        ...
        await waitFor(() => {
            ...
            // Match price with any kind of spaces/separators
            expect(screen.getByText((content) => content.match(/1.*000.*000/))).toBeInTheDocument();
            expect(screen.getByText(/GNF/i)).toBeInTheDocument();
            expect(screen.getByText(/Kipe/i)).toBeInTheDocument();
            expect(screen.getByText((content) => content.includes('Muslim'))).toBeInTheDocument();
        });
    });

    it('opens contact modal when logged in user clicks contact', async () => {
        api.get.mockResolvedValue({data: mockProperty});
        useAuthMock = {
            user: mockUser
        }; // Logged in
